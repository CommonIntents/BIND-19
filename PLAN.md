...
| 任务 | 内容 | 规范依据 | 状态 |
|---|---|---|---|
| R1 | 测试向量生成器（Rust 代码，生成 JSON 测试向量） | 规范冻结前置 | ⏳ 待开工 |
| R2 | PFP/SAP/帧编解码测试向量（≥10 组，含边界值） | 协议家族架构 | ⏳ |
| R3 | 防重放/规则6/密钥轮换/CATASTROPHIC 测试向量（≥10 组） | 规则 1-7 | ⏳ |
| R4 | PAH 签名测试向量（已知密钥+消息→已知签名+截断值） | 规则 5 | ⏳ |
| R5 | 测试向量文档发布（Markdown + JSON，其他项目可直接使用） | 生态友好 | ⏳ |
| R6 | 多租户场景验证测试（跨租户隔离 + 计数器分片） | 附录 D.1 | ⏳ |
| R7 | 示例代码（examples/ 目录，展示如何使用 bind19 crate） | 生态友好 | ⏳ |
