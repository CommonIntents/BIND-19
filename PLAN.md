...
| B2 | 防重放缓存集成到帧处理流程（Seq-Counter 查重 + 拒绝重放帧） | 规则 4（防重放） | ⏳ 待开工 |
| B3 | BIND-19 端到端集成测试（完整帧编解码 + PFP/SAP + 防重放 + 密钥轮换 + CATASTROPHIC） | 全规则 | ⏳ |
| B4 | 基准测试（criterion，防重放缓存性能 + 帧编解码性能 + PAH 验证性能） | 附录 D.3 | ⏳ |
| B5 | 多租户隔离验证（跨租户缓存/密钥/计数器隔离） | 附录 D.1 | ⏳ |
